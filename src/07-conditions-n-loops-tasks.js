...
function doRectanglesOverlap(rect1, rect2) {
  if (rect1.top + rect1.width < rect2.top || rect2.top + rect2.width < rect1.top) return false;
...
function isCreditCardNumber(ccn) {
  let nCheck = 0;
  let evenIndex = false;
  const cardArr = [...ccn.toString()];

  for (let n = cardArr.length - 1; n >= 0; n -= 1) {
    let nDigit = Number(cardArr[n]);
...
function getCommonDirectoryPath(pathes) {
  const set = new Set();
  pathes.forEach((el) => set.add(el[0]));
  ...
  for (let i = 0; i < arr.length; i += 1) {
    arr[i].forEach((el) => {
      if (firstPath.includes(el)) result.push(el);
    });
  }
  if (result.length === 0) return '';
  if (result.every((el) => el === '')) return '/';
  const path = result.join('/').trim();
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < position.length; i += 1) {
    rowsArr[i].push(position[0][i], position[1][i], position[2][i]);
    if (position[i][0] && position[i][0] === position[i][1] && position[i][0] === position[i][2]) {
      return position[i][0];
    }
  }
  for (let i = 0; i < rowsArr.length; i += 1) {
    if (rowsArr[i][0] && rowsArr[i][0] === rowsArr[i][1] && rowsArr[i][0] === rowsArr[i][2]) {
      return rowsArr[i][0];
    }
  }
  return undefined;
